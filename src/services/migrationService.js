...
} = require('../models/TeachingExperienceDetail');
...
const listUser = async () => {
  ...
  const userMaping = await Promise.all(
    user.data.data.map(async (o) => {
      let userType;
      let gender;
      let province;
      let teacherStatus;
      ...
      if (o.userImages && o.userImages.length > 0) {
        images = o.userImages.map((img) => {
          const dataImages = {
            ...
            temporaryIdentityId:
              img && img.identitiesId ? img.identitiesId : null,
            userId: 'User id',
        ...
        teachExp = o.userTeachingExperiences.map((teach) => {
          const dataTeachingExperienceDetail = {
            temporaryTeachingExperienceId:
              teach && teach._id ? teach._id : null,
          ...
          const dataTeachingExperience = {
            temporaryTeachingExperienceId:
              teach && teach._id ? teach._id : null,
            temporaryIdentityId:
              teach && teach.identitiesId ? teach.identitiesId : null,
      ...
      if (o.userEducationBackgrounds && o.userEducationBackgrounds.length > 0) {
        eduBackground = o.userEducationBackgrounds.map((edu) => {
          const dataEducationBackground = {
            ...
            temporaryIdentityId:
              edu && edu.identitiesId ? edu.identitiesId : null,
      ...
      const dataUser = {
        ...
        temporaryIdentityId:
          o.userIdentity && o.userIdentity._id ? o.userIdentity._id : null,
        email: o.user && o.user.email ? o.user.email : null,
        firstName:
          o.user && o.user.name && o.user.name.split(' ').length > 1
            ? o.user.name.split(' ')[0]
            : o.user.name,
        lastName:
          o.user && o.user.name && o.user.name.split(' ').length > 1
            ? o.user.name.split(' ').shift()
            : '',
      ...
      const dataUserDetail = {
        ...
        temporaryIdentityId:
          o.userIdentity && o.userIdentity._id ? o.userIdentity._id : null,
        birthPlace:
          o.userDetail && o.userDetail.birthPlace
            ? o.userDetail.birthPlace
            : null,
        birthDate:
          o.userDetail && o.userDetail.birthDate
            ? moment(o.userDetail.birthDate).format('YYYY-MM-DD')
            : null,
        religion:
          o.userDetail && o.userDetail.religion ? o.userDetail.religion : null,
        idCardType:
          o.userDetail && o.userDetail.cardType ? o.userDetail.cardType : null,
        idCardNumber:
          o.userDetail && o.userDetail.cardId ? o.userDetail.cardId : null,
        mailingAddress:
          o.userDetail && o.userDetail.address ? o.userDetail.address : null,
        city: o.userDetail && o.userDetail.city ? o.userDetail.city : null,
        region: province ? province : null,
        postalCode:
          o.userDetail && o.userDetail.postalcode
            ? parseInt(o.userDetail.postalcode)
            : null,
      ...
      const dataUserSchool = {
        ...
        temporaryIdentityId:
          o.userIdentity && o.userIdentity._id ? o.userIdentity._id : null,
        schoolName:
          o.userSchool && o.userSchool.name ? o.userSchool.name : null,
        schoolAddress:
          o.userSchool && o.userSchool.grade ? o.userSchool.grade : null,
      };
...
const addUserDetail = async () => {
  ...
  maping.forEach((item) => map.set(item.temporaryIdentityId, {
    ...map.get(item.temporaryIdentityId),
    ...item,
  }));
  // IDK
  ...
  for (const loop of merging) {
    ...
    if (loop.birthDate) {
      birthDate = moment(loop.birthDate).isValid() === true
        ? moment(loop.birthDate).format('YYYY-MM-DD')
        : moment('1999-01-01').format('YYYY-MM-DD');
    }
...
const addTeachingExperience = async () => {
  ...
  const data = fs.readFileSync(
    './public/Migration_Dec_16_21_15/teaching_exps.json',
    'utf-8',
  );
  const convertJson = JSON.parse(data);
  const maping = convertJson.map((o) => o);
  ...
  for (const loopTeaching of merging) {
    ...
    const dataTeachingExperience = {
      temporaryTeachingExperienceId:
        loopTeaching && loopTeaching._id.$oid ? loopTeaching._id.$oid : null,
      temporaryIdentityId:
        loopTeaching && loopTeaching.identitiesId
          ? loopTeaching.identitiesId
          : null,
      teacherId:
        loopTeaching && loopTeaching.dataValues.id
          ? loopTeaching.dataValues.id
          : null,
      universityName:
        loopTeaching && loopTeaching.school ? loopTeaching.school : null,
      univeristyCity:
        loopTeaching && loopTeaching.city ? loopTeaching.city : null,
      teachingStatus:
        loopTeaching && loopTeaching.status ? loopTeaching.status : null,
    ...
    const insertTeachingExperience = await TeachingExperience.create(
      dataTeachingExperience,
    );
  ...
  arrayTeachingExperienceDetail.forEach((item) => resultMap.set(item.temporaryTeachingExperienceId, {
    ...resultMap.get(item.temporaryTeachingExperienceId),
    ...item,
  }));
  ...
  for (const loopTeachingDetail of mergingResults) {
    const dataTeachingExperienceDetailResult = {
      ...
      temporaryTeachingExperienceId:
        loopTeachingDetail.temporaryTeachingExperienceId,
...
const addEducationBackground = async () => {
  const user = await User.findAll();
  const fileEduBackground = fs.readFileSync(
    './public/Migration_Dec_16_21_15/edu_backgrounds.json',
    'utf-8',
  );
  ...
  const mapIdentitiesId = filteringEducationBackground.map(
    (o) => o.identitiesId,
  );
  ...
  for (const loopEdu of merging) {
    ...
    const dataEdu = {
      temporaryIdentityId:
        loopEdu && loopEdu.identitiesId ? loopEdu.identitiesId : null,
      teacherId:
        loopEdu && loopEdu.dataValues.id ? loopEdu.dataValues.id : null,
      ...
      universityName:
        loopEdu && loopEdu.institution ? loopEdu.institution : null,
      thesisTitle: loopEdu && loopEdu.title ? loopEdu.title : '-',
      educationGpa: floatingScore ? parseFloat(floatingScore) : 0,
      educationFrom: loopEdu.fromDate
        ? converterFromDate
        : moment('01-01-2000', 'DD-MM-YYYY').format('DD-MM-YYYY'),
      educationTo: loopEdu.toDate
        ? converterToDate
        : moment('01-01-2000', 'DD-MM-YYYY').format('DD-MM-YYYY'),
...
const addBank = async () => {
  const user = await User.findAll();
  const fileBank = fs.readFileSync(
    './public/Migration_Dec_16_21_15/bank_accounts.json',
    'utf-8',
  );
  ...
  for (const loopBank of merging) {
    const data = {
      userId:
        loopBank && loopBank.dataValues.id ? loopBank.dataValues.id : null,
      ...
      bankOwnerName:
        loopBank && loopBank.bankOwnerName ? loopBank.bankOwnerName : null,
      temporaryIdentityId:
        loopBank && loopBank.identitiesId ? loopBank.identitiesId : null,
    };
...
const addDevice = async () => {
  const user = await User.findAll();
  const fileDevice = fs.readFileSync(
    './public/Migration_Dec_16_21_15/devices.json',
    'utf-8',
  );
  ...
  for (const loopDevice of merging) {
    const data = {
      userId:
        loopDevice && loopDevice.dataValues.id
          ? loopDevice.dataValues.id
          : null,
      registeredToken:
        loopDevice && loopDevice.registerdToken
          ? loopDevice.registerdToken
          : null,
    };
...
const addSubject = async () => {
  const fileSubject = fs.readFileSync(
    './public/Migration_Dec_16_21_15/mapels.json',
    'utf-8',
  );
  const dataSubject = JSON.parse(fileSubject);
  const mapSubject = dataSubject.map((o) => o);

  const distinctSubject = [...new Set(mapSubject.map((item) => item.name))];
...
const addGradeGroup = async () => {
  const fileGrade = fs.readFileSync(
    './public/Migration_Dec_16_21_15/kelas.json',
    'utf-8',
  );
  ...
  const distinctGrade = [
    ...new Map(mapGrade.map((item) => [item.name, item])).values(),
  ];
  ...
  for (const loopGradeGroup of distinctGrade) {
    ...
    const data = {
      temporaryGradeId:
        loopGradeGroup && loopGradeGroup._id.$oid
          ? loopGradeGroup._id.$oid
          : null,
      curriculumId: curriculum && curriculum.id ? curriculum.id : null,
      gradeGroupCode:
        loopGradeGroup && loopGradeGroup.name
          ? loopGradeGroup.name.toLowerCase()
          : null,
      gradeGroupName:
        loopGradeGroup && loopGradeGroup.name
          ? loopGradeGroup.name.toUpperCase()
          : null,
    };
